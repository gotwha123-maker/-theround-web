    ...
    <section id="talktalk" className="section talktalk-section" style={{ 
      ...
      <div className="container">
        <div className="section-header text-center reveal-on-scroll">
          ...
          <p className="section-lead" style={{ fontSize: "1.2rem", color: "var(--color-text-primary)", maxWidth: "800px", margin: "0 auto", lineHeight: "1.6", fontWeight: 500 }}>
        ...
        <div className="talktalk-grid" style={{ 
          ...
          {/* Action Cards with Premium Design */}
          {[
            ...
            <div key={item.id} className={`talk-card reveal-on-scroll delay-${idx*100}`} style={{ 
              ...
              boxShadow: "0 15px 35px rgba(0,0,0,0.05)", transition: "all 0.4s cubic-bezier(0.4, 0, 0.2, 1)", cursor: "default"
            }}
            onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-12px)"; e.currentTarget.style.boxShadow = "0 25px 50px rgba(13, 148, 136, 0.15)"; }}
            ...
            >
              <div>
                ...
                <h3 style={{ marginBottom: "1rem", fontSize: "1.6rem", fontWeight: 900, color: "var(--color-primary)" }}>{item.title}</h3>
                <p style={{ marginBottom: "2.5rem", color: "var(--color-text-primary)", fontSize: "1.05rem", lineHeight: "1.6" }}>{item.desc}</p>
              </div>
              <button onClick={() => openModal(item.id)} className="btn btn-primary btn-block btn-lg" style={{ borderRadius: "16px", padding: "1.1rem", fontSize: "1.1rem", fontWeight: 800 }}>지금 신청하기</button>
            </div>
          ))}
        </div>

        {/* Sub Menu with Pulse and Emerald styling */}
        <div className="talktalk-sub-menu reveal-on-scroll delay-300" style={{ 
          display: "flex", justifyContent: "center", gap: "2.5rem", marginTop: "5.5rem", flexWrap: "wrap",
          padding: "2.5rem", borderTop: "2px solid var(--color-primary)", background: "rgba(255,255,255,0.4)", borderRadius: "0 0 40px 40px"
        }}>
      ...
      <style jsx>{`
        .classic-link-btn {
            background: none; border: none; color: var(--color-primary); font-weight: 900;
            font-size: 1.15rem; cursor: pointer; transition: all 0.3s ease; padding: 0.5rem 1.2rem;
      ...
      {/* Modals remain structurally the same but with premium polish */}
      {modalType && (
        <div className="modal open" style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div className="modal-overlay" onClick={closeModal}></div>
          <div className="modal-container" style={{ maxWidth: "600px", width: "95%", maxHeight: "90vh", overflowY: "auto", borderRadius: "32px", boxShadow: "0 30px 100px rgba(0,0,0,0.25)" }}>
            ...
            <div className="modal-body" style={{ padding: "3.5rem" }}>
              ...
              <form onSubmit={handleSubmit} className="admin-form">
                <div style={{ marginBottom: "2.5rem" }}>
                  <h4 style={{ fontSize: "1.1rem", fontWeight: 800, marginBottom: "1.5rem", color: "var(--color-text-primary)", borderLeft: "4px solid var(--color-primary)", paddingLeft: "1rem" }}>신청자 정보</h4>
                ...
                {modalType === 'survey' && (
                  <div style={{ marginBottom: "2.5rem" }}>
                    <h4 style={{ fontSize: "1.1rem", fontWeight: 800, marginBottom: "1.5rem", color: "var(--color-text-primary)", borderLeft: "4px solid var(--color-primary)", paddingLeft: "1rem" }}>의뢰 상세</h4>
                    <div className="form-row" style={{ display: "flex", gap: "1.5rem", marginBottom: "1.5rem" }}>
                ...
                <div style={{ background: "rgba(13, 148, 136, 0.03)", padding: "1.5rem", borderRadius: "20px", marginBottom: "2.5rem", border: "1px dashed var(--color-primary)" }}>
                  <label style={{ display: "flex", alignItems: "flex-start", gap: "1rem", cursor: "pointer" }}>
                    ...
                    <span style={{ fontSize: "0.9rem", color: "var(--color-text-primary)", lineHeight: "1.6" }}>
