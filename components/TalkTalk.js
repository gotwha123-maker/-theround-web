    ...
    <section id="talktalk" className="section talktalk-section">
      <div className="container">
        ...
        <div className="talktalk-grid" style={{ 
          ...
          {/* Main Action Cards */}
          <div className="talk-card reveal-on-scroll" style={{ background: "var(--color-bg-secondary)", padding: "2.5rem", borderRadius: "24px", border: "1px solid var(--color-border)", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
            <div>
              <h3 style={{ marginBottom: "1rem", color: "var(--color-primary)", fontWeight: 800 }}>나눔 요청하기</h3>
              <p style={{ marginBottom: "2rem", color: "var(--color-text-muted)" }}>물품, 화장품, 의류 등 소중한 마음을 나누고 싶은 분들의 신청을 기다립니다.</p>
            </div>
            <button onClick={() => openModal('giving')} className="btn btn-primary btn-block">나눔 신청하기</button>
          </div>

          <div className="talk-card reveal-on-scroll delay-100" style={{ background: "var(--color-bg-secondary)", padding: "2.5rem", borderRadius: "24px", border: "1px solid var(--color-border)", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
            <div>
              <h3 style={{ marginBottom: "1rem", color: "var(--color-primary)", fontWeight: 800 }}>구인 요청하기</h3>
              <p style={{ marginBottom: "2rem", color: "var(--color-text-muted)" }}>한반도의 미래를 함께 만들어갈 인재를 찾으시는 기관 및 기업의 요청을 받습니다.</p>
            </div>
            <button onClick={() => openModal('job')} className="btn btn-primary btn-block">구인 요청하기</button>
          </div>

          <div className="talk-card reveal-on-scroll delay-200" style={{ background: "var(--color-bg-secondary)", padding: "2.5rem", borderRadius: "24px", border: "1px solid var(--color-border)", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
            <div>
              <h3 style={{ marginBottom: "1rem", color: "var(--color-primary)", fontWeight: 800 }}>설문조사 의뢰</h3>
              <p style={{ marginBottom: "2rem", color: "var(--color-text-muted)" }}>학술 연구 및 정책 제안을 위한 설문조사 대상자 모집 및 분석 지원을 의뢰하세요.</p>
            </div>
            <button onClick={() => openModal('survey')} className="btn btn-primary btn-block">조사 의뢰하기</button>
          </div>
        </div>

        {/* Small Menu Items - Simple but High Visibility */}
        <div className="talktalk-sub-menu reveal-on-scroll delay-300" style={{ display: "flex", justifyContent: "center", gap: "2rem", marginTop: "3.5rem", flexWrap: "wrap", padding: "1.5rem", borderTop: "1.5px solid var(--color-primary)" }}>
              ...
              <form onSubmit={handleSubmit} className="admin-form">
                ...
                {modalType === 'survey' && (
                  <div style={{ marginBottom: "2rem" }}>
                    ...
                    <div className="form-group" style={{ marginBottom: "1rem" }}><label>조사 목적/제목 *</label><input type="text" name="surveyPurpose" value={formData.surveyPurpose} onChange={handleInputChange} required /></div>
                  </div>
                )}
                ...
                <div style={{ background: "#f8fafc", padding: "1.2rem", borderRadius: "16px", marginBottom: "2rem" }}>
                  <label style={{ display: "flex", alignItems: "flex-start", gap: "0.8rem", cursor: "pointer" }}>
                    ...
                    <span style={{ fontSize: "0.85rem", color: "var(--color-text-muted)", lineHeight: "1.6" }}>
                      ...
                      보내주신 정보는 더라운드 네트워크 관리 및 활동 안내를 위해 소중하게 사용됩니다.
                    </span>
                  </label>
                </div>
