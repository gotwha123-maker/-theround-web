...
// Forced to use provided key without 'path' check
...
async function run() {
  ...
  if (base) {
    ...
    for (const item of verified) {
      ...
      } catch (e) { console.error(`  ! Airtable Error (${item.title}):`, e.message); }
    }
  } else {
    console.warn('[Airtable] Credentials missing or invalid, skipped sync.');
  }
}
